...
// a companion setting (MQUBE_AUTH_SERVER_URL) was left empty. The
// public MCP surface must not be exposed to that failure mode. Here
// there is no way to construct a Verifier that silently accepts every
// request — the type does not exist.
package auth
